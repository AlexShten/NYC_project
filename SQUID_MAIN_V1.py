...
def Create_connection():
    ...
    if connection_for_data_and_variables==None:
        try:
            connection_for_data_and_variables=psycopg2.connect(host=server_host, database=server_dbname, user=server_username, password=server_password, connect_timeout=2)#, connect_timeout=1, options='-c statement_timeout=1000')
            if connection_for_data_and_variables!=None:
...
def Request_error_to_server():
    ...
    while True:
        ...
        if ((write_error_thread_status==0) or (write_error_thread_status==1)) and check_thread_status==0:
            ...
            error_list=(error_sn, error_time, error_boilercurrent, error_ics1, error_ics2, error_ics3, error_t1, error_t2, error_t3, error_t4, error_t5, error_t6, error_t7, error_wt, error_relay1, error_relay2, error_relay3, error_relay4, error_relay5, error_relay6, error_relay7)
       
            try:
                connection_for_errors=psycopg2.connect(host=server_host, database=server_dbname, user=server_username, password=server_password)
    ...
    while True:
        if db_thread_status==1:
            ...
            if to_db_status==1:
                ...
                data=(data_sn, data_time, data_zone, data_boilerpumpfunamps, data_ics1, data_ics2, data_ics3, data_t1, data_t2, data_t3, data_t4, data_t5, data_t6, data_t7, data_ps, data_rt1, data_rt2, data_rt3, data_boiler, data_wt)
#                 try:
                print(data)
...
#                     Print_error("Data_to_DB", e)
            ...
            if from_db_status==1:
                DB_check("cycle")
                try:
                    text="SELECT sn,time,zone,boilerpumpfunamps,ics1,ics2,ics3,t1,t2,t3,t4,t5,t6,t7,ps,rt1,rt2,rt3,boiler FROM devicedata WHERE rowID>="+str(Quant)
                ...
                try:
                    ...
                    try:
                        connection_to_server_db=psycopg2.connect(host=server_host, database=server_dbname, user=server_username, password=server_password)
                        ...
                        for row in res:
                            ...
                            cursor_to_server_db.execute('INSERT INTO \
                                ...
                                VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);',tuple(inner_db_data_list_copy))
...
def Check_connection():
    ...
    while True:
        ...
        if check_thread_status==1:
            ...
            try:
                check_connection=psycopg2.connect(host=server_host, database=server_dbname, user=server_username, password=server_password, connect_timeout=2)
            except psycopg2.OperationalError as e:
                retries=0
            try:    
                if check_connection:
